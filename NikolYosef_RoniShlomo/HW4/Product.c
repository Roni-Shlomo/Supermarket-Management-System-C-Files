...
{
	// 1) Re-derive the type from the barcode prefix:
	//    i.e. "FR" => eFridge, "FV" => eFruitVegtable, etc.
	eProductType realType = eFridge; // fallback
	if (strlen(pProduct->barcode) >= 2)
	{
		char prefix[3];
		prefix[0] = (char)toupper((unsigned char)pProduct->barcode[0]);
		prefix[1] = (char)toupper((unsigned char)pProduct->barcode[1]);
		prefix[2] = '\0';

		if (strcmp(prefix, "FV") == 0) realType = eFruitVegtable;
		else if (strcmp(prefix, "FR") == 0) realType = eFridge;
		else if (strcmp(prefix, "FZ") == 0) realType = eFrozen;
		else if (strcmp(prefix, "SH") == 0) realType = eShelf;
	}

	// 2) Convert date to string (same as before)
	char* dateStr = getDateStr(&pProduct->expiryDate);

	// 3) Print everything
	printf("%-20s %-10s\t", pProduct->name, pProduct->barcode);
	printf("%-20s %5.2f %13d %7s %15s\n",
		typeStr[realType],   // <-- Use our re-derived type
		pProduct->price,
		pProduct->count,
		" ",
		dateStr);
